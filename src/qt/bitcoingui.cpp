...
void BitcoinGUI::createActions()
{
    ...
    blockAction = new QAction(QIcon(fUseBlackTheme ? ":/icons/black/block" : ":/icons/block"), tr("Block Explorer"), this);
    blockAction->setToolTip(tr("Explore the BlockChain"));
    blockAction->setCheckable(true);
#ifdef Q_OS_MAC
        blockAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_4));
#else
        blockAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_4));
#endif
...
#ifdef Q_OS_MAC
        addressBookAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("eMark Core") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("eMark Core"));
...
void BitcoinGUI::restoreWindowGeometry()
{
    ...
    QSize size = settings.value("nWindowSize", QSize(1010, 550)).toSize();
    if (!pos.x() && !pos.y())
    {
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid eMark address or malformed URI parameters."));
    }
